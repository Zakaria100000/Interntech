            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">Delegates Details</h2>
                        <a href="delegate-create.php" class="btn btn-success float-right">Add New Delegate</a>
                        <a href="delegate-index.php" class="btn btn-info float-right mr-2">Reset View</a>
                        <a href="index.php" class="btn btn-secondary float-right mr-2">Back</a>
                    </div>

                    <div class="form-row">
                        <form action="delegate-index.php" method="get">
                        <div class="col">
                    ...
                    $count_pages = "SELECT * FROM user WHERE id_role = 3 ";
                    ...
                    if(!empty($_GET['search'])) {
                        ...
                        $sql = "SELECT u.id as id, u.firstname as firstname, u.lastname as lastname, u.email as email, u.password as password, r.name as role, c.name as center, p.name as promo 
                            ...
                            INNER JOIN center as c ON u.id_center = c.id
                            INNER JOIN promo as p ON u.id_promo = p.id
                            WHERE CONCAT_WS (u.id,u.firstname,u.lastname,u.email,u.password,r.name,c.name,p.name) 
                            LIKE '%$search%' AND u.id_role = 3
                            ORDER BY $order $sort
                            LIMIT $offset, $no_of_records_per_page";
                        $count_pages = "SELECT * FROM user
                            WHERE CONCAT_WS (id,firstname,lastname,email,password,id_role,id_center,id_promo) AND id_role = 3 
                            LIKE '%$search%'
                            ORDER BY $order $sort";
                    }
                    else {
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    ...
                                    echo "<td>" . $row['firstname'] . "</td>";echo "<td>" . $row['lastname'] . "</td>";echo "<td>" . $row['email'] . "</td>";echo "<td style='-webkit-text-security: disc;'>" . $row['password'] . "</td>";echo "<td>" . $row['role'] . "</td>";echo "<td>" . $row['center'] . "</td>";echo "<td>" . $row['promo'] . "</td>";
                                        echo "<td>";
                                            echo "<a href='delegate-read.php?id=". $row['id'] ."' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                                            echo "<a href='delegate-update.php?id=". $row['id'] ."' title='Update Record' data-toggle='tooltip'><i class='far fa-edit'></i></a>";
                                            echo "<a href='delegate-delete.php?id=". $row['id'] ."' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
